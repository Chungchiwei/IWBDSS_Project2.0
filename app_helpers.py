...
def get_port_full_info(port_code: str, crawler: Any) -> PortDisplayInfo:
    ...
    # 相容新版 crawler 回傳 PortInfo dataclass 或舊版 dict
    if hasattr(port_info, "latitude"):
        lat = port_info.latitude
        lon = port_info.longitude
        name       = port_info.name
        country    = port_info.country
        station_id = port_info.station_id
    else:
        lat        = port_info.get("latitude", 0.0)
        lon        = port_info.get("longitude", 0.0)
        name       = port_info.get("port_name", "N/A")
...
def calculate_tug_capacity(
    ...
    Args:
        ...
        hp_per_tug:  每艘拖船馬力
    """
    bp_ton   = MOORING.tug_bollard_pull(hp_per_tug)   # 使用 MooringConstants 計算
...
def normalize_dataframe(analyzer: Any, vessel_info: Any) -> pd.DataFrame:
    ...
    優化重點：每筆記錄只呼叫一次 calculate_wind_force，
    避免原版重複呼叫 5 次的效能浪費。

    Args:
        analyzer:    WeatherAnalyzer 實例
    ...
    for rec in analyzer.data:
        ...
        row: Dict[str, Any] = {
            ...
            "wind_speed_kts": getattr(rec, "wind_speed_kts", rec.wind_speed),
            "wind_gust_kts":  getattr(rec, "wind_gust_kts",  rec.wind_gust),
            "wind_speed_ms":  getattr(rec, "wind_speed_ms",  knots_to_ms(rec.wind_speed)),
            "wind_gust_ms":   getattr(rec, "wind_gust_ms",   knots_to_ms(rec.wind_gust)),
            "wind_dir_deg":   getattr(rec, "wind_dir_deg",   compass_to_degrees(rec.wind_direction)),
            "wave_direction": rec.wave_direction,
            "wave_height":    rec.wave_height,
            "wave_max":       rec.wave_max,
            "wave_period":    rec.wave_period,
            "wave_sig_m":     getattr(rec, "wave_sig_m",     rec.wave_height),
            "wave_max_m":     getattr(rec, "wave_max_m",     rec.wave_max),
            "wave_period_s":  getattr(rec, "wave_period_s",  rec.wave_period),
        }

        # ── 風力計算（每筆只算兩次：平均風速 + 陣風）──
        avg_res  = calculate_wind_force(row["wind_speed_ms"], row["wind_dir_deg"], vessel_info.wind_area, cd, berth_angle)
        gust_res = calculate_wind_force(row["wind_gust_ms"],  row["wind_dir_deg"], vessel_info.wind_area, cd, berth_angle)
        ...
        row.update({
            ...
            "risk_level":    classify_wind_risk(row["wind_speed_kts"], row["wind_gust_kts"]),
        })
        rows.append(row)
